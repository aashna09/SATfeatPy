...
cpdef dict get_graph_stats(str name, list node_degrees, list weights=[]):
    ...
    node_val_rate = np.count_nonzero([deg == node_mode for deg in node_degrees]) / len(node_degrees)
    ...
    weights_val_rate = np.count_nonzero([w == weights_mode for w in weights]) / len(weights)
